...
impl PartsUnknown {
    ...
    pub fn move_player(&mut self, x: i32, y: i32) -> Array /* Event[] */ {
        self.world.move_player(Hex { x, y }).into_iter()
    ...
    pub fn end_turn(&mut self) -> Array /* Event[] */ {
        self.world.end_turn().into_iter()
            .map(display::Event::new)
            .map(JsValue::from)
            .collect()
    }
